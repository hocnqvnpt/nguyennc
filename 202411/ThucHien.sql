...
delete from ptm_daily_cops_dthu where thang_ptm=202411;
...
select thang_ptm,ma_tb,ma_gd,ten_tb,dthu_goi,ma_nguoigt,cast(null as varchar(10))duoctinh,ma_pb,ma_to,manv_ptm ma_nv,thang_ptm thang
 ...
 where thang_ptm=202411
   ...
   and exists (select * from ttkd_bsc.blkpi_danhmuc_kpi_vtcv where thang=202411 and ma_kpi='HCM_SL_DAILY_003' and ma_vtcv=a.ma_vtcv);
...
update bangluong_kpi a
   set tyle_thuchien=round(thuchien/giao*100,2)
      ,mucdo_hoanthanh=least(round(thuchien/giao*100,2),120)
       --select * from bangluong_kpi a
       --update bangluong_kpi a set thuchien=''
 where thang=202411 and ma_kpi='HCM_SL_DAILY_003' and giao>0;
 

----------------------------------------HCM_DT_PTMOI_056
--create table nguyennc.ct_bsc_ptmoi_056_202411_l1 as select * from ct_bsc_ptmoi_056 where thang=202411;

delete from ct_bsc_ptmoi_056 where thang=202411;
...
select 202411,ma_pb,ma_to,ma_nv,ma_vtcv
 ...
 where thang=202411-- and tinh_bsc=1
   and exists(select * from blkpi_danhmuc_kpi_vtcv 
               where thang=202411 and ma_kpi='HCM_DT_PTMOI_056' and to_truong_pho is null and giamdoc_phogiamdoc is null 
                 and ma_vtcv=a.ma_vtcv)
;
...
update bangluong_kpi a
   set diem_tru=(select tong_diem_tru_bsc from ct_bsc_ptmoi_056 where thang=a.thang and ma_nv=a.ma_nv)
       ...
       --select * from bangluong_kpi a
 where thang=202411 and ma_kpi='HCM_DT_PTMOI_056'
   and exists(select * from blkpi_danhmuc_kpi_vtcv 
               where thang=202411 and ma_kpi='HCM_DT_PTMOI_056' and to_truong_pho is null and giamdoc_phogiamdoc is null
                 and ma_vtcv=a.ma_vtcv)
...
----------------------------------------HCM_DT_PTMOI_054
delete from ct_dvs_pkhdn_2024 where thang_ptm=202411;
...
select thang_ptm,ma_gd,ma_tb,ma_kh,ten_tb,dich_vu
  ...
  from ttkd_bsc.ct_bsc_ptm a
 where thang_ptm=202411 and dthu_goi<>0
...
update bangluong_kpi a
 ...
 where thang=202411 and ma_kpi='HCM_DT_PTMOI_054'
   and exists (select * from blkpi_danhmuc_kpi_vtcv 
                where thang=202411 and upper(ma_kpi)='HCM_DT_PTMOI_054' and to_truong_pho is null and giamdoc_phogiamdoc is null
                  and ma_vtcv=a.ma_vtcv);
...
update bangluong_kpi a
 ...
 where thang=202411 and ma_kpi='HCM_DT_PTMOI_054'
   and exists (select * from blkpi_danhmuc_kpi_vtcv 
                where thang=202411 and upper(ma_kpi)='HCM_DT_PTMOI_054' and to_truong_pho is not null
                  and ma_vtcv=a.ma_vtcv);
...
select dthu_goi
      ,(select ma_nv from blkpi_dm_to_pgd 
         where thang=202411 and ma_pb in ('VNP0702300','VNP0702400','VNP0702500') and ma_kpi='HCM_DT_PTMOI_054' and ma_to=a.ma_to)ma_nv
...
select dthu_goi
      ,(select ma_nv from blkpi_dm_to_pgd 
         where thang=202411 and ma_pb in ('VNP0702300','VNP0702400','VNP0702500') and ma_kpi='HCM_DT_PTMOI_054'
...
select (select distinct ma_nv from blkpi_dm_to_pgd 
      ...
      ,th_luyke_tong
  from nguyennc.ktkh_bcao_9a_th a
 where thang=202411 and stt='1.1'
)group by ma_nv;
...
update bangluong_kpi a set tyle_thuchien=round(thuchien/giao*100,2)
       --   update bangluong_kpi set tyle=''
       --   select * from bangluong_kpi a
 where thang=202411 and ma_kpi='HCM_DT_LUYKE_003' and giao>0;
...
select ma_nv,round(sum(th_luyke_ptm)/1000000,3)dthu 
  from(select (select distinct ma_nv from blkpi_dm_to_pgd 
                where thang=202411 and ma_pb in ('VNP0702300','VNP0702400','VNP0702500') and ma_kpi='HCM_DT_PTNAM_006'
                  and ma_to=a.ma_to)ma_nv
             ,th_luyke_ptm
         from nguyennc.ktkh_bcao_9a_th a
        where thang=202411 and stt='0'
...
update bangluong_kpi a set tyle_thuchien=round(thuchien/giao*100,2)
       --   update bangluong_kpi set tyle=''
       --   select * from bangluong_kpi a
 where thang=202411 and ma_kpi='HCM_DT_PTNAM_006' and giao>0;
...
----------------------------------------HCM_CL_DHQLY_007
--create table ct_bsc_cl_dhqly_007_bhol_202411_l1 as select * from ct_bsc_cl_dhqly_007_bhol where thang=202411;

delete from ct_bsc_cl_dhqly_007_bhol where thang=202411;
...
select distinct a.thang,a.ma_pb,a.ten_pb,a.ma_to,a.ten_to,b.ma_nv,a.ma_nv ma_nhanvien,a.ten_nv ten_nhanvien,a.ma_vtcv,a.ten_vtcv
 ...
 where a.thang=202411 and a.tinh_bsc=1 and a.thaydoi_vtcv=0 and a.thang=b.thang and a.ma_to=b.ma_to
   and exists(select * from blkpi_danhmuc_kpi_vtcv 
               where thang=202411 and ma_kpi='HCM_CL_DHQLY_007' and ma_vtcv='VNP-HNHCM_BHKV_52'
...
update bangluong_kpi a
   set (diem_cong,diem_tru)=(select diem_cong,diem_tru 
      ...
      --    select * from bangluong_kpi a                           
 where thang=202411 and ma_kpi='HCM_CL_DHQLY_007'
   and exists (select * from blkpi_danhmuc_kpi_vtcv 
                where thang=202411 and upper(ma_kpi)=upper(a.ma_kpi) and ma_vtcv='VNP-HNHCM_BHKV_52' and to_truong_pho is not null
                  and ma_vtcv=a.ma_vtcv)
...
----------------------------------------HCM_CL_DHQLY_006 -- TT
--create table ct_bsc_cl_dhqly_006_tt_202411_l1 as select * from ct_bsc_cl_dhqly_006_tt where thang=202411;

delete from ct_bsc_cl_dhqly_006_tt where thang=202411;
...
update ct_bsc_cl_dhqly_006_tt a
 ...
 where thang=202411 
   and exists(select * from blkpi_danhmuc_kpi_vtcv 
               where thang=202411 and ma_kpi='HCM_CL_DHQLY_006' and to_truong_pho is not null
                 and ma_vtcv=a.ma_vtcv)
   and exists(select * from nhanvien where thang=a.thang and tinh_bsc=1 and thaydoi_vtcv=0 and ma_nv=a.ma_nv)
;                              
...
update bangluong_kpi a
   set (diem_cong,diem_tru)=(select diem_cong,diem_tru 
      ...
      -- update bangluong_kpi set tyle_thuchien=''   
      --    select * from bangluong_kpi a                          
 where thang=202411 and ma_kpi='HCM_CL_DHQLY_006'
   and exists (select * from blkpi_danhmuc_kpi_vtcv 
                where thang=202411 and upper(ma_kpi)=upper(a.ma_kpi) and to_truong_pho is not null and ma_vtcv<>'VNP-HNHCM_BHKV_17'
                  and ma_vtcv=a.ma_vtcv);
...
--create table ct_bsc_cl_dhqly_006_pgd_202411_l1 as select * from ct_bsc_cl_dhqly_006_pgd where thang=202411;

delete from ct_bsc_cl_dhqly_006_pgd where thang=202411;
insert into ct_bsc_cl_dhqly_006_pgd
select 202411 thang,ma_pb,ma_nv,ma_nhanvien
...
select ma_pb
  ...
  from dinhmuc_giao_dthu_ptm a
 where thang=202411 and nhombrcd_dtgiao>0
   and exists (select * from blkpi_danhmuc_kpi_vtcv 
...
select ma_pb
      ...
      ,null nhomcntt_dtgiao,null nhomcntt_kqth,null nhomtsl_dtgiao,null nhomconlai_kqth
  from dinhmuc_giao_dthu_ptm a
 where thang=202411 and nhomvina_dtgiao>0
   and exists (select * from blkpi_danhmuc_kpi_vtcv 
                where thang=a.thang and upper(ma_kpi)='HCM_DT_PTMOI_021' and to_truong_pho is null and giamdoc_phogiamdoc is null
                  and ma_vtcv=a.ma_vtcv)
   and exists(select * from nhanvien where thang=a.thang and tinh_bsc=1 and thaydoi_vtcv=0 and ma_nv=a.ma_nv)
...
select ma_pb
  ...
  from dinhmuc_giao_dthu_ptm a
 where thang=202411 and nhomcntt_dtgiao>0
   and exists (select * from blkpi_danhmuc_kpi_vtcv 
                where thang=a.thang and upper(ma_kpi)='HCM_DT_PTMOI_021' and to_truong_pho is null and giamdoc_phogiamdoc is null
                  and ma_vtcv=a.ma_vtcv)
   and exists(select * from nhanvien where thang=a.thang and tinh_bsc=1 and thaydoi_vtcv=0 and ma_nv=a.ma_nv)
...
select ma_pb
  ...
  from dinhmuc_giao_dthu_ptm a
 where thang=202411 and nhomtsl_dtgiao>0
   and exists (select * from blkpi_danhmuc_kpi_vtcv 
                where thang=a.thang and upper(ma_kpi)='HCM_DT_PTMOI_021' and to_truong_pho is null and giamdoc_phogiamdoc is null
                  and ma_vtcv=a.ma_vtcv)               
   and exists(select * from nhanvien where thang=a.thang and tinh_bsc=1 and thaydoi_vtcv=0 and ma_nv=a.ma_nv)
...
update ct_bsc_cl_dhqly_006_pgd
 ...
 where thang=202411 and tong_dthu_giao>0;
...
---- PGD phu trach CSKH
/*
create table ct_bsc_cl_dhqly_006_pgd_cskh_sai as
select * from ct_bsc_cl_dhqly_006_pgd 
 where thang=202411 and ma_nv in ('VNP016659','VNP017585','VNP017947','VNP017729','VNP016898','VNP017496','VNP016983');

delete from ct_bsc_cl_dhqly_006_pgd 
 where thang=202411 and ma_nv in ('VNP016659','VNP017585','VNP017947','VNP017729','VNP016898','VNP017496','VNP016983');
*/
...
select 202411,ma_pb--,ten_pb
  ...
  from bangluong_kpi a
 where a.thang=202411 and a.ma_kpi='HCM_DT_PTMOI_021' and a.ma_vtcv not in ('VNP-HNHCM_BHKV_28','VNP-HNHCM_BHKV_52')
...
update ct_bsc_cl_dhqly_006_pgd a
 ...
 where thang=202411 and ma_nhanvien=' TONG CONG';   
...
update bangluong_kpi a
 ...
 where thang=202411 and ma_kpi='HCM_CL_DHQLY_006'
   and exists (select * from blkpi_danhmuc_kpi_vtcv 
                where thang=202411 and upper(ma_kpi)='HCM_CL_DHQLY_006' and giamdoc_phogiamdoc is not null
                  and ma_vtcv=a.ma_vtcv);
...
delete from ct_nghiepvu_sbh where thang=202411;
insert into ct_nghiepvu_sbh
select distinct 202411 thang,ma_gd,ma_tb,ma_kh,loai,ten_loaihd,ngay_cn
      ...
      ,manv_ra_pct ma_nv
      ,(select ma_to from nhanvien where thang=202411 and ma_nv=a.manv_ra_pct)ma_to
      ,(select ma_pb from nhanvien where thang=202411 and ma_nv=a.manv_ra_pct)ma_pb
  from tuyenngo.SBH_GIAODICH_202411_ct_new a 
 where exists(select * from blkpi_danhmuc_kpi_vtcv where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_vtcv=a.ma_vtcv)  
;
...
update ct_nghiepvu_sbh a set khong_tinh='Trung thao tac ben ccbs'
 where thang=202411
   ...
   and exists(select * from ct_nghiepvu_sbh where thang=202411 and loai='VNP - CCBS' and ten_loaihd='DOI SIM' and ma_tb=a.ma_tb);


update ct_nghiepvu_sbh a set khong_tinh='Trung nghiep vu khac cua cung thue bao trong ky'
 where thang=202411
   and ten_loaihd='CAP NHAT DB'
   and exists(select * from ct_nghiepvu_sbh where thang=202411 and ten_loaihd<>'CAP NHAT DB' and ma_tb=a.ma_tb);


update ct_nghiepvu_sbh a set khong_tinh='Trung nghiep vu khac cua cung thue bao trong ky'
 where thang=202411
   and ten_loaihd in ('DONG MO DV|0','DONG MO DV|1')
   and exists(select * from ct_nghiepvu_sbh 
               where thang=202411 and ten_loaihd not in ('DONG MO DV|0','DONG MO DV|1') 
                 and ma_tb=a.ma_tb);



update ct_nghiepvu_sbh a set khong_tinh='Trong tien trinh LAP DAT MOI'
--select * from ct_nghiepvu_sbh a
 where thang=202411
   and ten_loaihd in ('KY LAI HOP DONG GOC')
   and exists(select * from ct_nghiepvu_sbh 
               where thang=202411 and ten_loaihd='LAP DAT MOI'
                 and ma_tb=a.ma_tb);


create index ct_nghiepvu_ghtt_1 on ct_nghiepvu_ghtt(ma_tb,ma_gd);
drop table ct_nghiepvu_ghtt;
create table ct_nghiepvu_ghtt as

delete from ct_nghiepvu_ghtt where thang=202411;
insert into ct_nghiepvu_ghtt
select distinct 202411 thang,b.ma_tb,a.ma_gd
 ...
 where kieuld_id in (551,550,24,13080) and c.khoanmuctt_id=11 and c.tien>0 and to_char(ngay_yc,'yyyymm')='202411';


update ct_nghiepvu_sbh a set khong_tinh='Trong tien trinh GHTT'
--select * from ct_nghiepvu_sbh a
 where thang=202411
   and ten_loaihd in ('DAT COC MOI','TAO MOI GOI DA DICH VU','THAY DOI DAT COC','THAY DOI GOI DA DICH VU'
                         ,'THAY DOI TOC DO INTERNET','TIEP NHAN KHAO SAT DAT MOI','TIEP NHAN LAP DAT MOI')
   and exists (select * from ct_nghiepvu_ghtt where thang=202411 and ma_tb=a.ma_tb and ma_gd=a.ma_gd);
...
select ma_nv,ma_tb,''ma_gd,1 sl
 ...
 where thang=202411 and khong_tinh is null and nhom_tinh=1
 
  union all
select ma_nv,ma_tb,ma_gd,least(count(*),2)sl
  from ct_nghiepvu_sbh 
 where thang=202411 and khong_tinh is null and nhom_tinh=2
 group by ma_nv,ma_tb,ma_gd
 
  union all
select ma_nv,ma_tb,ma_gd,least(count(*),1)sl
  from ct_nghiepvu_sbh 
 where thang=202411 and khong_tinh is null and nhom_tinh=3
 group by ma_nv,ma_tb,ma_gd 
 
 union all
select ma_nv,'','',count(distinct ma_kh)*0.25 sl
  from ct_nghiepvu_sbh a
 where thang=202411 and khong_tinh is null and nhom_tinh=5
 group by ma_nv
 ;
...
-- cap nhat cho thang 11/2024 theo VB 4752/TTr-KHDN1-NVC ngay 22/10/2024
update bangluong_kpi set thuchien=thuchien+297 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_nv='VNP017943';
update bangluong_kpi set thuchien=thuchien+286 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_nv='VNP017944';
...
------dieu cchinh cccho DN3
create table bangluong_kpi_202411_dn3 as
 select * from bangluong_kpi a
 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500';

select * from bangluong_kpi_202411_dn3;

update bangluong_kpi_202411_dn3 set thuchien=5247 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP017049';
update bangluong_kpi_202411_dn3 set thuchien=5454 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP028838';
update bangluong_kpi_202411_dn3 set thuchien=3791 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP028221';
update bangluong_kpi_202411_dn3 set thuchien=4433 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP020229';
update bangluong_kpi_202411_dn3 set thuchien=4307 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP019903';
update bangluong_kpi_202411_dn3 set thuchien=3396.5 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP028231';
update bangluong_kpi_202411_dn3 set thuchien=2592.25 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP027782';
update bangluong_kpi_202411_dn3 set thuchien=3434.75 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP029933';


update bangluong_kpi_202411_dn3 a
   set tyle_thuchien=round(thuchien/giao*100,2)
      ,mucdo_hoanthanh=least(round(thuchien/giao*100,2),120)
       --   update bangluong_kpi_202411_dn3 a set tyle_thuchien='',mucdo_hoanthanh=''
       --   select * from bangluong_kpi_202411_dn3 a
 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001';
 
 
select MA_NV,	TEN_NV,	MA_VTCV,	TEN_VTCV,	MA_TO,	TEN_TO,	MA_PB,	TEN_PB, thuchien HCM_SL_HOTRO_001
  from bangluong_kpi_202411_dn3;
 



update bangluong_kpi set thuchien=5247 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP017049';
update bangluong_kpi set thuchien=5454 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP028838';
update bangluong_kpi set thuchien=3791 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP028221';
update bangluong_kpi set thuchien=4433 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP020229';
update bangluong_kpi set thuchien=4307 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP019903';
update bangluong_kpi set thuchien=3396.5 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP028231';
update bangluong_kpi set thuchien=2592.25 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP027782';
update bangluong_kpi set thuchien=3434.75 where thang=202411 and ma_kpi='HCM_SL_HOTRO_001' and ma_pb='VNP0702500' and ma_nv='VNP029933';
...
drop table SBH_GIAODICH_202411_CT_new;
create table SBH_GIAODICH_202411_CT_new as 
select ma_gd,ma_tb,ma_kh,ten_loaihd,loai,nhomtinh,ngay_cn,rownum stt,cast(null as varchar(100))khong_tinh
      ,manv_ra_pct ma_nv
      ,(select ma_to from nhanvien where thang=202411 and ma_nv=b.manv_ra_pct)ma_to
      ,(select ma_pb from nhanvien where thang=202411 and ma_nv=b.manv_ra_pct)ma_pb 
  from(select distinct ma_gd,ma_tb,ma_kh,manv_ra_pct,ten_loaihd,loai,ngay_cn
        ...
        where manv_ra_pct is not null
          and exists (select * from blkpi_danhmuc_kpi_vtcv 
                       where thang=202411 and upper(ma_kpi)='HCM_SL_CSKHH_003' and ma_vtcv=a.ma_vtcv)
        order by ten_loaihd,loai,ngay_cn)b;

update SBH_GIAODICH_202411_CT_new set khong_tinh='Khong co thong tin: ma giao dich, ma KH, ma thue bao'
 where ma_gd is null and ma_kh is null and ma_tb is null;

update SBH_GIAODICH_202411_CT_new set khong_tinh='thao tac tu ID163,164'
 where (ma_tb,ma_nv) in (select case when length(b.matb)=9 and lpad(b.matb,2) in ('81','82','83','84','85','88','91','94')
                          ...
                          where a.thang=202411 and a.id_yckh=b.id_yckh);
...
update SBH_GIAODICH_202411_CT_new a set khong_tinh='Trong tien trinh GHTT'
 where exists(select * from temp_giahan_tratruoc where ma_tb=a.ma_tb and ma_gd=a.ma_gd);
   
 

update SBH_GIAODICH_202411_CT_new a set khong_tinh='Trong tien trinh lap dat moi'
 where exists(select * from SBH_GIAODICH_202411_CT_new 
...
select * FROM ttkdhcm_ktnv.gdv_haumai a
 where exists (select 1 
  ...
  and to_char(ngay_yc,'yyyymm')='202411';
  
  
update SBH_GIAODICH_202411_CT_new a set khong_tinh='KH co yc thao tac khac'
 where loai='VNP - CCBS' and ten_loaihd='THU CUOC'
   and exists(select * from SBH_GIAODICH_202411_CT_new where ten_loaihd not in ('THU CUOC','CAP NHAT DB') and ma_kh=a.ma_kh);
 
 
update SBH_GIAODICH_202411_CT_new a set khong_tinh='KH co yc thao tac khac'
 where loai='THU CUOC ONEBSS' and ten_loaihd='THU CUOC'
   and exists(select * from SBH_GIAODICH_202411_CT_new where ten_loaihd!='THU CUOC' and ma_kh=a.ma_kh);

update SBH_GIAODICH_202411_CT_new a set khong_tinh='KH da duoc nhan tin'
 where loai='THU CUOC ONEBSS' and ten_loaihd='THU CUOC'
   and exists(select * from ttkdhcm_ktnv.gdv_haumai where to_char(ngay_yc,'yyyymm')='202411' and no_sms=1 and ma_tb=a.ma_tb);

update SBH_GIAODICH_202411_CT_new a set khong_tinh='KH da duoc nhan tin (dai)'
 where loai='THU CUOC ONEBSS' and ten_loaihd='THU CUOC'
...
update SBH_GIAODICH_202411_CT_new a set khong_tinh='KH co yc thao tac khac'
 where loai='ONEBSS' and ten_loaihd='THU KHAC'
   and exists(select * from SBH_GIAODICH_202411_CT_new where ten_loaihd!='THU KHAC' and ma_kh=a.ma_kh);
...
update SBH_GIAODICH_202411_CT_new a set khong_tinh='Trung khi dong bo doi sim ve ccbs'
  --select * from SBH_GIAODICH_202411_CT_new a
 where loai='USSD' and ten_loaihd='DOI SIM'
   and exists(select * from SBH_GIAODICH_202411_CT_new where loai='VNP - CCBS' and ten_loaihd='DOI SIM' and ma_tb=a.ma_tb);



update SBH_GIAODICH_202411_CT_new a set khong_tinh='Cap nhat DB da dong bo ben Nghiep vu khac'
 where loai='VNP - CCBS' and ten_loaihd='CAP NHAT DB'
   and exists(select * from SBH_GIAODICH_202411_CT_new 
               where loai='VNP - CCBS' and ten_loaihd<>'CAP NHAT DB' 
...
------- Chuyen doi VCC
alter table SBH_GIAODICH_202411_CT_new add chuyen_vcc varchar(100);

update SBH_GIAODICH_202411_CT_new a 
   set chuyen_vcc=1
        --select * from SBH_GIAODICH_202411_CT_new a
 where exists(select * from ttkd_bsc.dt_ptm_vnp_202411_bs where somay=a.ma_tb)
   and ten_loaihd='DANG KY HUY CHUYEN DOI GOI CUOC';
                    
                    
select * from SBH_GIAODICH_202411_CT_new 
...
select distinct tenkieu_ld from ttkd_bsc.ct_bsc_ptm 
                    where thang_ptm=202411 and loaitb_id=149


alter table SBH_GIAODICH_202411_CT_new add ten_tb varchar(400);
alter table SBH_GIAODICH_202411_CT_new add ma_dt_kh varchar(2);
...
select ma_to,ma_nv,count(distinct ma_kh)*0.25 sl,loai,ten_loaihd
  from SBH_GIAODICH_202411_CT_new a
 where loai='VNP - CCBS' and ten_loaihd='THU CUOC'
...
select ma_to,ma_nv,count(*)sl,loai,ten_loaihd
 ...
 union all
select ma_to,ma_nv,count(*)sl,loai,ten_loaihd
  from (select * from SBH_GIAODICH_202411_CT_new where khong_tinh is null)a
 where nhomtinh=2 -- nhom 2: lay thao tac cuoi cung trong ngay
   and stt=(select max(stt) from SBH_GIAODICH_202411_CT_new 
             where nhomtinh=2 
...
-- bo sung PGD Binh Chanh   thang 11/2024
-- sogiao=6697     select sum(dmuc_haumai_ch) from dinhbien_cuahang where thang=202411 and ma_pb='VNP0701100'

update bangluong_kpi a
   set thuchien=(select sum(sl)*1.1 from tmp_ct_gdv where ma_to in ('VNP0701180','VNP0701190','VNP07011A0'))
      ,giao=6697,ngaycong=21,tytrong=20,tyle_thuchien=94.38,mucdo_hoanthanh=94.38 -------------------
       --   update bangluong_kpi a set thuchien=''
       --   select sum(giao),count(*) from bangluong_kpi a
       --   select * from bangluong_kpi a
 where thang=202411 and ma_kpi='HCM_SL_CSKHH_003'
